<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=baiviet&act=delete_danhmuc&listid=" + listid;
	});
	});
	...
	{	
		...
		window.location ="index.php?com=baiviet&act=man_danhmuc&type=&type=<?=$_REQUEST['type']?>&key="+a;	
		return true;
	}
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=baiviet&act=man_danhmuc&type=<?=$_REQUEST['type']?>"><span>Danh mục cấp 1</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=baiviet&act=add_danhmuc&type=<?=$_GET['type']?>'" />
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
   ...
   <tr>
   		...
   		<td class="title_name_data">
            <a href="index.php?com=baiviet&act=edit_danhmuc&id=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='.$_REQUEST['curPage'];?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?>" class="tipS SC_bold">
			<?php
                    $d->reset();
                    $sql="select ten from #_baivietdanhmuc_lang where id_baiviet_danhmuc = '".$items[$i]['id']."' order by id asc";
                    $d->query($sql);
        ...
        <td class="actBtns">
            <a href="index.php?com=baiviet&act=edit_danhmuc&id=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='.$_REQUEST['curPage'];?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=baiviet&act=delete_danhmuc&id=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='.$_REQUEST['curPage'];?>" onClick="if(!confirm('Xác nhận xóa: <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
   </tr>
