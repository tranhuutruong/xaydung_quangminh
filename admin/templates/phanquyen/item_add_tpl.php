<?php
    ...
    $d->reset();
    $sql = "select id,ten from #_product_danhmuc order by id desc";
    $d->query($sql);
    $row_danhmuc = $d->result_array();
?>
<style>
...
<script type="text/javascript">
    $(document).ready(function(){
      ...
      <?php if($item['id_danhmuc']!=''){?>
            var id_danhmuc = <?=$item['id_danhmuc']?>;
        <?php } else { ?>
            var id_danhmuc = '';
        <?php } ?>
        <?php if($item['id_list']!=''){?>
...
<div class="widget">
    ...
    <div class="formRow">
        ...
        <div class="formRight">
            ...
            <select id="id_danhmuc" name="id_danhmuc[]" class="main_font" multiple="multiple">
                    <?php 
                    $row_danhmuc1 = json_decode($item['id_danhmuc']);
                    for($i=0;$i<count($row_danhmuc);$i++){ ?>   
                    <option value="<?=$row_danhmuc[$i]['id']?>" <?php if($row_danhmuc1!=''){if(in_array($row_danhmuc[$i]['id'],$row_danhmuc1)){?> selected="selected"<?php } } ?>> - <?=$row_danhmuc[$i]['ten']?></option>
                    <?php } ?>
            </select><br /><br />
