...
<?php
	...
	$d->reset();
	$sql_quan = "select id,ten,id_item from #_thanhpho where id_item='".$_REQUEST['thanhpho_item']."' limit 0,1";
	$d->query($sql_quan);
	$quan = $d->fetch_array();	
...
function get_phuong()
	...
	$d->reset();
	$sql_phivanchuyen="select phivanchuyen,id_item from #_thanhpho where id='".$_GET['thanhpho']."'limit 0,1";
...
<form name="supplier" id="validate" class="form" action="index.php?com=order&act=save" method="post" enctype="multipart/form-data">
	<div class="widget">
        ...
        <div class="formRow">
         ...
         <div class="formRow">
			<label>Email</label>
			<div class="formRight">
             <input type="text" name="email" title="Email khách hàng" style="width:80% !important"  id="email" class="tipS validate[required] read" value="<?=@$item['email']?>" />
			</div>
			<div class="clear"></div>
		</div>	
    ...
    <tbody>
     <?php      
				...
				$pid=$chitietdonhang[$i]['id_sanpham'];
//
//				$pname=get_product_name($pid);
//				$sale=get_price_sale($pid);
//				$dongia=get_price2($pid);
        ...
        <tr id="productct<?=$chitietdonhang[$i]['id']?>">
          ...
          <td><?=$chitietdonhang[$i]['ten']?></td>
           <td align="center"><img src="<?=_upload_sanpham.$pphoto?>" height="100"  /></td>
