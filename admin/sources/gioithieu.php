...
function get_items(){
	...
	if(@$_REQUEST['noibat']!='')
	{
		...
		$sql_sp = "SELECT id,noibat FROM table_gioithieu where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$noibat=$cats[0]['noibat'];
		if($noibat==0)
		{
			$sqlUPDATE_ORDER = "UPDATE table_gioithieu SET noibat=1 WHERE id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}
		else
		{
			$sqlUPDATE_ORDER = "UPDATE table_gioithieu SET noibat=0  WHERE  id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");	
		}	
	}
	//===========================================================
	...
	$sql = "select * from #_gioithieu where id<>0 ";
	if($_REQUEST['id_item']!='')
	{
	...
	$url = "index.php?com=gioithieu&act=man&id_item=".$_REQUEST['id_item']."&curPage=".$_REQUEST['curPage'];
	$maxR = 20;
	$maxP = 4;
...
function save_item(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data))			
				redirect("index.php?com=gioithieu&act=man&id_item=".$_REQUEST['id_item']."&curPage=".$_REQUEST['curPage']);
		else
...
function get_cats(){
	...
	{
		$id_up = $_REQUEST['hienthi'];
		$sql_sp = "SELECT id,hienthi FROM table_gioithieu_item where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
		$sqlUPDATE_ORDER = "UPDATE table_gioithieu_item SET hienthi =1 WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
		$sqlUPDATE_ORDER = "UPDATE table_gioithieu_item SET hienthi =0  WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	//===========================================================
	...
	{
		$id_up = $_REQUEST['noibat'];
		$sql_sp = "SELECT id,noibat FROM table_gioithieu_item where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$noibat=$cats[0]['noibat'];
	if($noibat==0)
	{
		$sqlUPDATE_ORDER = "UPDATE table_gioithieu_item SET noibat =1 WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
		$sqlUPDATE_ORDER = "UPDATE table_gioithieu_item SET noibat =0  WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	
	$sql = "select * from #_gioithieu_item where id<>0 ";
	if($_REQUEST['key']!='')
	{
		$sql.=" and ten like '%".$_REQUEST['key']."%'";
	}
...
function delete_cat(){
	...
	if(isset($_GET['id']))
	{
		...
		$sql = "select id,thumb,photo from #_gioithieu where id_item='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0)
		{
			while($row = $d->fetch_array())
			...
			$sql = "delete from #_gioithieu where id_item='".$id."'";
			$d->query($sql);
		}
